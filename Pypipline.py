...
class bio_pipline:
    total_log = "Total_infomation.log.txt"
    ...
    def get_dict(configure_file):
        ...
        for line in all_content:
            ...
            if c_type == "i":
                ...
                if "from-file" in content.lower():
                    ...
                    print(f"Succefully read {len(set(content))} items from file: {in_file_name} in Program[{p_number}]")
                else:
        ...
        for p in config_dict.keys():
            ...
            config_dict[p]["i"] = ["Just_Shell_Command"] if not config_dict[p]["i"] else config_dict[p]["i"]
            config_dict[p]["t"] = config_dict[p].get("t", "1")

            if config_dict[p]["i"] != ["Just_Shell_Command"]:
    ...
    def run_popen(p_name, subgroup_n, cmd_dict):
        ...
        if show_only:
            for sample, cmd in cmd_dict.items():
                sample = "Just_Shell_Command" if sample == "Just_Shell_Command" else sample
                print(f"Sample: {sample}:  {cmd}")
        else:
            for sample, cmd in cmd_dict.items():
                if sample != "Just_Shell_Command":
                    p_log = (f"Program-{p_name}_group-{subgroup_n}_sample-{sample.replace(' ', '_')}.log.txt")
                else:
                    sample = "Just_Shell_Command"
    ...
    def run_cmd_per_program(p_name, run_dict, ):
        ...
        if show_only:
            for subgroup_n, cmd in run_dict.items():
                n_sample = len(cmd.keys()) if list(cmd.keys()) != ["Just_Shell_Command"] else 0
    ...
    def show_single_program(p_name):
        ...
        empty_sample = True if s_dict[p_name]["i"] == ['Just_Shell_Command'] else False
        ...
        for each in s_dict[p_name]["p"]:
            ...
            if each.count(temp_label) == 0 and s_dict[p_name]["i"] != ['Just_Shell_Command']:
